...
.. code-block:: none
   ...
   mlpstorage <closed|open|whatif> <benchmark> <model|algorithm> <command> <file|object> ...
        ...
        - The second argument is the workload benchmark to run: <checkpointing|training>
...
FCR submissions are required to contain two sets of results for each of the following four benchmarks, with one set for each of the two (#Hosts : #Accelerators) combinations. Each run must adhere to the configuration parameters in the sub-bullets to each benchmark specification below.
...
* Checkpointing: Two results are reported for each Checkpointing test (save, load):
...
.. code-block::
    ...
    [METRIC] Checkpoint save duration (seconds): 19.6279 (0.8521)
    [METRIC] Checkpoint save I/O Throughput (GiB/second): 5.3436 (0.2093)
    [METRIC] ==========================================================
    ...
    [METRIC] ==========================================================
    [METRIC] Number of Simulated Accelerators: 8
    [METRIC] Checkpoint load duration (seconds): 9.9284 (0.3288)
    [METRIC] Checkpoint load I/O Throughput (GiB/second): 10.5570 (0.3337)
    [METRIC] ==========================================================
..
...
.. code-block:: bash
    ...
    (mlpstorage) $ TMPDIR=${chkpt_dir} mlpstorage closed checkpointing run file \
                   ...
                   --model llama3-8b \
                   --hosts node1 node2 node node4 node5 node6 node7 node8 \
                   --num-processes 8 \
    ...
    (mlpstorage) $ sudo clush -b -w node[1-8] 'echo 3 > /proc/sys/vm/drop_caches'
...
Sample S3 Object I/O-based Checkpoint test on one node with split phases:
-------------------------------------------------------------------------
...
Sample POSIX File I/O-based Training run on one node:
-----------------------------------------------------
...
Sample S3 Object I/O-based Training run on 10 nodes:
------------------------------------------------------------------------
...
.. code-block:: bash
    ...
    # Note that objects will be created, rather than files
        ...
        ...
        2026-09-17 20:55:11|RESULT: Minimum file count dictated by dataset size to memory size ratio.
        2026-09-17 20:55:11|RESULT: Number of training files: 41891481
        2026-09-17 20:55:11|RESULT: Number of training subfolders: 0
        2026-09-17 20:55:11|RESULT: Total disk space required for training: 12600.00GiB
        2026-09-17 20:55:11|RESULT: Run the following command to generate data:
        mlpstorage closed training retinanet datagen file --hosts node1 node2 node3 node4 node5 node6 node7 node8 node9 node10 --exec-type=mpi --num-processes=40 --results-dir=${results_dir} --data-dir=<INSERT_DATA_DIR> --systemname=${systemname} --params dataset.num_files_train=41891481 dataset.num_subfolders_train=0 dataset.total_disk_bytes=13529147029317 dataset.skip_listing=True dataset.listing_validation_interval=10000
        ...
    ...
    (mlpstorage) $ mlpstorage closed training retinanet datagen object \
                   ...
                   --data-dir ${data_dir} \
                   --systemname ${systemname} \
                   --params dataset.num_files_train=41891481 \
                     dataset.num_subfolders_train=0 \
                     dataset.total_disk_bytes=13529147029317 \
                     dataset.skip_listing=True \
                     dataset.listing_validation_interval=10000
...
Valid runs show the Figures of Merit for Checkpointing and Training runs, and will have Training results show :code:`train_au_meet_expectation: success`.
