...
# Set Build Locations
export DIR_MINIEM_INSTALL="${DIR_MINIEM_INSTALL:-`pwd -P`/install}"
export DIR_MINIEM_SRC="${DIR_MINIEM_SRC:-`pwd -P`/miniem}"
mkdir -p "${DIR_MINIEM_INSTALL}"

# Create symlinks just in case
if test ! -h Trilinos ; then
    rm -rf Trilinos
    ln -s miniem Trilinos
fi
if test ! -h trilinos ; then
    rm -rf trilinos
    ln -s miniem trilinos
...
ENABLE_TEST_BINARIES=true \
    ...
    CMAKE_INSTALL_PREFIX="${DIR_MINIEM_INSTALL}" \
    TRILINOS_SRC="${DIR_MINIEM_SRC}" \
