...
reference variant. The way in which the run scripts, described below, execute
the code, the reference variant is the baseline sequential (CPU) variant of
each kernel.
...
The process for generating results for the Priority 2 kernels is eseentially
...
in our application that are immportant rather than key numerical kernels.
